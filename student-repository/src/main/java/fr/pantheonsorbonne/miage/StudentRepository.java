...
public class StudentRepository implements Iterable<Student> {
	...
	private StudentRepository(String db) {this.db = db;}

	public static StudentRepository withDB(String db) throws FailedToFindException {
		if (!Files.exists(Paths.get(db))) {
			throw new FailedToFindException("failed to find" + Paths.get(db).toAbsolutePath().toString());
		}
	...
	public StudentRepository add(Student s) throws FailedToUpdateException, FailedToFindException {
