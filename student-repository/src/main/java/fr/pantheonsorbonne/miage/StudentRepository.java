		...
		try (FileWriter writer = new FileWriter(this.db)) {
			...
			previousContent.forEachRemaining(student -> {
				try {
					...
					throw new StudentRepositoryUpdateDbFileStudentException("failed to update db file");
				}
			});
			...
			throw new StudentRepositoryAddStudentException("failed to update db file");
		}
		return this;
