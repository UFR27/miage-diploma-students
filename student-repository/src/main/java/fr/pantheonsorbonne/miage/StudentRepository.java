...
public class StudentRepository implements Iterable<Student> {
	...
	public java.util.Iterator<Student> iterator() {
		try (FileReader reader = new FileReader(this.db)) {
			...
			currentIterator = parser.getRecords().stream().map(reccord -> new Student(Integer.parseInt(reccord.get(2)),
					reccord.get(0), reccord.get(1), reccord.get(3))).map(c -> c).iterator();
