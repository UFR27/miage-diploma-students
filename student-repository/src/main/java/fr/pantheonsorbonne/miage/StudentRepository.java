...
public class StudentRepository implements Iterable<Student> {
	...
	public static StudentRepository withDB(String db) {
		if (!Files.exists(Paths.get(db))) {
			throw new DiplomaException("failed to find" + Paths.get(db).toAbsolutePath().toString());
		}
		...
		try (FileWriter writer = new FileWriter(this.db)) {
			...
			previousContent.forEachRemaining(student -> {
				try {
					...
					throw new DiplomaException("failed to update db file");
				}
			});
	...
	public java.util.Iterator<Student> iterator() {
		try (FileReader reader = new FileReader(this.db)) {
			...
			currentIterator = parser.getRecords().stream().map(reccord -> new Student(reccord.get(1),reccord.get(0), Integer.parseInt(reccord.get(2)), reccord.get(3))).map(c -> c).iterator();
