...
public class StudentRepository implements Iterable<Student> {
	...
	public static List<String> toRecord(Student stu) {
	...
	public StudentRepository add(Student s) {
		...
		try (FileWriter writer = new FileWriter(this.db)) {
			...
			previousContent.forEachRemaining(student -> {
				try {
					csvFilePrinter.printRecord(toRecord(student));
				} catch (IOException e) {
					throw new UpdateFileException("failed to update db file ", e);
				}
			});
			csvFilePrinter.printRecord(toRecord(s));
