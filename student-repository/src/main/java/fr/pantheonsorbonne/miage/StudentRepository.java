...
public class StudentRepository implements Iterable<Student> {
	...
	public static StudentRepository withDB(String db) throws FailedToUpdateException {
		if (!Files.exists(Paths.get(db))) {
			throw new FailedToUpdateException("failed to find" + Paths.get(db).toAbsolutePath().toString());
		}
