...
public class StudentRepository implements Iterable<Student> {
	...
	public static StudentRepository withDB(String db) throws FailedException {
		if (!Files.exists(Paths.get(db))) {
			throw new FailedException("failed to find" + Paths.get(db).toAbsolutePath().toString());
		}
...
>>>>>>> encryption
	...
	public StudentRepository add(Student s) throws FailedException {
		...
		try (FileWriter writer = new FileWriter(this.db)) {
			...
			throw new FailedException("failed to update db file");
		}
		return this;
