...
public abstract class AbstractDiplomaGenerator implements DiplomaGenerator {
	...
	public InputStream getContent() {
		...
		try (ByteArrayOutputStream bos = new ByteArrayOutputStream();) {
			...
			throw new DocGenFailException("failed to generate the doc to stream to", e);
		}

	}

	protected void writeToStream(OutputStream os) throws DocGenFailException {
		...
		try {
			...
			throw new FileGenException("failed to generate the doc to stream to", e);
		} finally {
			document.close();
		}
	}
