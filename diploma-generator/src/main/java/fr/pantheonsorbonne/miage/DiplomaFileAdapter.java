...
public class DiplomaFileAdapter extends FileGenerator<AbstractDiplomaGenerator> {
	...
	@Override
	public void generateFile(String outputFile) throws FailedWriteException {
		try (FileOutputStream fos = new FileOutputStream(outputFile)) {
			InputStream is = this.generator.getContent();
			ByteStreams.copy(is, fos);
			is.close();
		} catch (IOException | FailedGenerateFile e) {
			e.printStackTrace();
			throw new FailedWriteException("failed to write diploma file");
		}
	}
