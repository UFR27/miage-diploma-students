...
public class DiplomaFileAdapter extends FileGenerator<AbstractDiplomaGenerator> {
	...
	@Override
	public void generateFile(String outputFile) throws WriteDiplomaFileException, GenerateFileException, GenerateDocumentException {
		try (FileOutputStream fos = new FileOutputStream(outputFile)) {
			InputStream is = this.generator.getContent();
			ByteStreams.copy(is, fos);
			is.close();
		} catch (IOException e) {
			throw new WriteDiplomaFileException();
		}
	}
