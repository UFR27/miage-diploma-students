...
public class DiplomaFileAdapter extends FileGenerator<AbstractDiplomaGenerator> {
	...
	@Override
	public void generateFile(String outputFile) throws WriteFileException {
		try (FileOutputStream fos = new FileOutputStream(outputFile)) {
			InputStream is = this.generator.getContent();
			ByteStreams.copy(is, fos);
			is.close();
		} catch (IOException | GenerateFileException e) {
			e.printStackTrace();
			throw new WriteFileException("failed to write diploma file", e);
		}
	}
