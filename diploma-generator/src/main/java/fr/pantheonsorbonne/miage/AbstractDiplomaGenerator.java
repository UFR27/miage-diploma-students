...
public abstract class AbstractDiplomaGenerator implements DiplomaGenerator {
	 ...
	 */
	@Override
	public InputStream getContent() throws ExecutionException {
		...
		try (ByteArrayOutputStream bos = new ByteArrayOutputStream();) {
			...
			throw new ExecutionException("failed to generate the file to stream to", e);
		}

	}

	protected void writeToStream(OutputStream os) throws ExecutionException {
		...
		try {
			...
			throw new ExecutionException("failed to generate Document", e);
		} finally {
			document.close();
		}
	}
