...
public abstract class AbstractDiplomaGenerator implements DiplomaGenerator {
	...
	public void getContent() throws FileUpdateException {
		...
		try (ByteArrayOutputStream bos = new ByteArrayOutputStream();) {
			...
			throw new FileUpdateException("failed to generate the file to stream to", e);
		} catch (FileUpdateGenerateException e) {
	...
	protected void writeToStream(OutputStream os) throws FileUpdateGenerateException {
		...
		try {
			...
			throw new FileUpdateGenerateException("failed to generate Document", e);
		} finally {
			document.close();
		}
	}
