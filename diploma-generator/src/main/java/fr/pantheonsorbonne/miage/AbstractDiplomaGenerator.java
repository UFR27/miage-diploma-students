...
public abstract class AbstractDiplomaGenerator implements DiplomaGenerator {
	 ...
	 */
	@Override
	public InputStream getContent() throws FileException {
		...
		try (ByteArrayOutputStream bos = new ByteArrayOutputStream();) {
			...
			throw new FileException("failed to generate the file to stream to",e);
		}

	}

	protected void writeToStream(OutputStream os) throws FileException {
		...
		try {
			...
			throw new FileException("failed to generate Document",e);
		} finally {
			document.close();
		}
	}
