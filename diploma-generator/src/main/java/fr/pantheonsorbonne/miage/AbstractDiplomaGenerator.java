...
public abstract class AbstractDiplomaGenerator implements DiplomaGenerator {
	 ...
	 */
	@Override
	public InputStream getContent() throws GenerateFail{
		...
		try (ByteArrayOutputStream bos = new ByteArrayOutputStream();) {
			...
			throw new GenerateFail("failed to generate the file to stream to", e);
		}

	}

	protected void writeToStream(OutputStream os) throws GenerateFail{
		...
		try {
			...
			throw new GenerateFail("failed to generate the file to stram to", e);
		} finally {
			document.close();
		}
	}
