...
public abstract class AbstractDiplomaGenerator implements DiplomaGenerator {
	...
	public InputStream getContent() throws GetContentException {
		...
		} catch (IOException | WriteToStreamException e) {

			throw new GetContentException("failed to generate the file to stream to", e);
		}

	}

	protected void writeToStream(OutputStream os) throws WriteToStreamException {
		...
		try {
			...
			throw new WriteToStreamException("failed to generate Document", e);
		} finally {
			document.close();
		}
	}
