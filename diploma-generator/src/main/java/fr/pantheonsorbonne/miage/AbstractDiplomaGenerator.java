...
public abstract class AbstractDiplomaGenerator implements DiplomaGenerator {
	 ...
	 */
	@Override
	public InputStream getContent() throws FailedGenerateFile {
		...
		} catch (IOException | FailedGenerateDocException e) {

			throw new FailedGenerateFile("failed to generate the file to stream to");
		}
	}

	protected void writeToStream(OutputStream os) throws FailedGenerateDocException{
		...
		try {
			...
			throw new FailedGenerateDocException("failed to generate Document");
		} finally {
			document.close();
		}
	}
