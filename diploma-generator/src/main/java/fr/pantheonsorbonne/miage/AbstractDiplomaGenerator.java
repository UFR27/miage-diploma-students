...
public abstract class AbstractDiplomaGenerator implements DiplomaGenerator {
	...
	public InputStream getContent() throws FailedGeneratingEncryptedFileException {
		...
		try (ByteArrayOutputStream bos = new ByteArrayOutputStream();) {
			...
			throw new FailedGeneratingFileException("failed to generate the file to stream to", e);
		}
	...
	protected void writeToStream(OutputStream os) {
		...
		try {
			...
			throw new FailedGeneratingDocumentException("failed to generate Document", e);
		} finally {
			document.close();
		}
	}
