...
public abstract class AbstractDiplomaGenerator implements DiplomaGenerator {
	 ...
	 * @return
	 */
	abstract public Collection<DiplomaSnippet> getDiplomaSnippets();
	...
	public InputStream getContent() {
		...
		try (ByteArrayOutputStream bos = new ByteArrayOutputStream();) {
			...
			throw new Exception("failed to generate the file to stream to", e);
		}
	...
	protected void writeToStream(OutputStream os) {
		...
		try {
			...
			throw new Exception("failed to generate Document", e);
		} finally {
			document.close();
		}
	}
