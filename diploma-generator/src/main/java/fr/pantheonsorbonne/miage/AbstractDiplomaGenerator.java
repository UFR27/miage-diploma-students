...
public abstract class AbstractDiplomaGenerator implements DiplomaGenerator {
	 ...
	 */
	@Override
	public InputStream getContent() throws DiplomaFailed {
		...
		try (ByteArrayOutputStream bos = new ByteArrayOutputStream();) {
			...
			throw new DiplomaFailed("failed to generate the file to stream to", e);
		}

	}

	protected void writeToStream(OutputStream os) throws DiplomaFailed {
		...
		try {
			...
			throw new DiplomaFailed("failed to generate Document", e);
		} finally {
			document.close();
		}
	}
